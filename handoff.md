...
## 2026-07-22 Step L17: macro expander with host macros landed

`src/smd/smdlisp/macroexpand/{expander.hpp,expander.test.cpp,CMakeLists.txt}` are new, wired into `src/smd/smdlisp/CMakeLists.txt` via `add_subdirectory(macroexpand)` plus a link edit (`target_link_libraries(smdlisp.smdlisp PUBLIC ... smdlisp.macroexpand)`). New CMake target `smdlisp.macroexpand` (STATIC, header-only, mirrors `smdlisp.reader`/`smdlisp.elaborator`'s pattern), linking `PUBLIC smdscheme.foundation smdlisp.reader smdlisp.elaborator`. This step ran in a separate worktree from L12 (Track A, `setq`/`defun`), per the plan's parallelism summary ("Track C ... runs parallel to L14–L16 after L11"); it touches only the new `macroexpand/` directory plus the one shared top-level `src/smd/smdlisp/CMakeLists.txt` (an `add_subdirectory` line and a link entry) — `git diff --stat` confirms `src/smd/smdlisp/reader`, `src/smd/smdlisp/elaborator`, `src/smd/smdlisp/closure`, and `src/smd/smdscheme/**` are all untouched by this step.

**Architecture: a datum-to-datum pass, exactly as decision D9 specifies.** The expander operates entirely on `reader::datum_type<MaxNodes,MaxList>` trees (L6) — it never constructs or references an `elaborator::core_type` node except at the very end, where `expand_and_elaborate<MaxNodes,MaxList>(datum, datum_arena, core_arena) -> result<core_type<MaxNodes,MaxList>>` (the new pipeline entry point, mirroring `elaborate()`'s own shape exactly, per the plan's suggestion) calls `expand_datum` first and then hands the *expanded* datum tree to the unmodified `elaborator::elaborate`. No elaborator or evaluator code changed at all this step — `git diff --stat -- src/smd/smdlisp/elaborator src/smd/smdlisp/closure` is empty. Expansion output is written into the *same* datum arena the reader populated (the simplest integration `handoff-next.md` flagged as an option, and the one this step took): every new node the expander builds (an `IF`/`PROGN`/`LET`/`COND` list, a fresh symbol, a `QUOTE` node for a `case` key) is allocated via `smdscheme::foundation::make_arena_box` into the caller's `datum_arena`, alongside the reader's original nodes.

**`host_macro<MaxNodes,MaxList>` registry, exactly matching the plan's sketch shape** (`name` + a `(call-form, arena) -> result<datum>` function pointer typedef `macro_fn`). `default_macro_table<MaxNodes,MaxList>() -> std::array<host_macro<MaxNodes,MaxList>,6>` holds all six macros; there is no separate `find_macro` helper — `macroexpand_1` inlines the linear scan and calls the matching macro's `expand` function in the same loop iteration it finds the match in. **This was a forced redesign, not a style choice:** an earlier draft had `find_macro` return a bare `macro_fn` (or `nullptr` if absent) and had `macroexpand_1` compare that return value against `nullptr` before calling it. GCC16 rejected this with `error: '(smd::smdlisp::macroexpand::detail::expand_cond<128, 16> == 0)' is not a constant expression` when the whole call chain (`expand_and_elaborate` → `expand_datum` → `macroexpand` → `macroexpand_1`) was evaluated inside a `static_assert` — comparing a function-pointer value obtained this way against `nullptr` is not usable in a manifestly-constant-evaluated context on this compiler, even though the pointer is perfectly well-defined at runtime. Caught by `make compile` failing on `expander.test.cpp`'s merge-criteria `static_assert`, not guessed. **Durable fact for L18/L19 (backquote, `defmacro`) and any later step that does table-driven dispatch by function pointer inside a `constexpr`/`static_assert` context: do not split "find a function pointer" and "check whether it was found" into two steps with a null-pointer comparison in between; find-and-call in the same loop iteration instead.**

**Six ordinary `host_macro` entries, no ad-hoc special-casing** (the plan's explicit test of whether the expander's shape is right): `COND`, `WHEN`, `UNLESS`, `AND`, `OR`, `CASE`, each a template function in `detail::` (`expand_cond`, `expand_when`, etc.) with the exact `macro_fn` signature, registered by address in `default_macro_table`. None needed a special code path in `macroexpand_1`/`macroexpand`/`expand_datum` beyond the shared, macro-agnostic dispatch machinery — confirming the plan's "if implementing these six requires ad-hoc special-casing ... the expander's API needs adjusting" concern did not materialize.

**Expansion shapes** (all via small `detail::make_list_v`/`make_list_from`/`make_symbol`/`make_quote` datum-builder helpers, new this step): `when`/`unless` → `(IF test (PROGN body...) NIL)` / `(IF test NIL (PROGN body...))`, requiring at least one body expression (matching the same "at least one body expression" restriction the elaborator already places on `lambda`/`progn`/`let`/`let*`, not a new cut). `and`/`or` → `(and)`⇒`T`, `(or)`⇒`NIL`, one-arg passthrough, else right-nested `if` chains; `or`'s multi-arg case (and `cond`'s test-only-clause case, and `case`'s keyform) needs single evaluation of a test-and-return expression, achieved via a `let`-bound reserved name (`%OR-TEMP`/`%COND-TMP`/`%CASE-TMP`) rather than a real `gensym` — **see `docs/divergences/DIV-0006-macro-temp-name-capture.md`, filed this step**: this is a genuine, if narrow, ANSI CL divergence (a user variable literally named e.g. `%OR-TEMP` inside an `or` form could be shadowed), permanent until L18/L19 bring real `gensym`/backquote machinery. `cond` → right-to-left nested `if`/`let` (test-only clauses use the same reserved-temp technique as `or`); zero clauses ⇒ `NIL`. `case` → `(LET ((%CASE-TMP keyform)) (COND (test1 body1...) ...))`, where each clause's test is `(EQL %CASE-TMP 'key)` for a single key, `(OR (EQL %CASE-TMP 'k1) (EQL %CASE-TMP 'k2) ...)` for a list of keys, or bare `T` for a `T`/`OTHERWISE` default clause (rejected as an error if not the last clause); keys are wrapped in a *native* `reader::datum_quote` node (the same kind `'x` produces), not a synthesized `(QUOTE key)` list, since case keys are literal data and the reader already has a dedicated node for exactly that.

**Recursive, whole-tree expansion (`expand_datum`), not just a single `macroexpand` call at the root.** The plan's "datum-to-datum pass" language implies macro calls must be found and expanded wherever they occur — inside `lambda` bodies, `if` branches, nested macro expansions (`case` expanding into a `let`/`cond` combination whose `cond` is a macro too) — not only at the top level. `expand_datum` walks the whole tree: at each list position it runs `macroexpand` (the fixpoint loop) once at the head, then recurses into whatever list results (never re-running `macroexpand` on the same node twice, so composing macros terminates without a second outer fixpoint loop across the tree). `#'` (`datum_function`) targets are recursed into (code); quoted data (`datum_quote`, the reader's `'x`) is **not** — returned unchanged, matching ANSI CL's rule that `quote` suppresses macroexpansion of its argument. **A correctness gap was found and closed before this was tested against the merge criteria:** the ANSI-legal long spelling `(quote (when x 1))` (an ordinary list headed by the symbol `QUOTE`, as opposed to the reader's `'(when x 1))` shorthand which lowers to a dedicated `datum_quote` node) was *not* initially protected — the generic "it's a list, recurse into elements" path would have macro-expanded inside literal data. Fixed by adding `detail::is_quote_form` (checks whether a list's head is a symbol spelled `QUOTE`) and consulting it before both recursing into `d`'s own children and before recursing into a post-macroexpansion result's children. Both spellings are covered by `ExpanderTest - QuoteShorthandSuppressesExpansion` and `ExpanderTest - QuoteListFormSuppressesExpansion`; `ExpanderTest - ExpansionReachesLambdaBody` pins that a nested, non-quoted macro call is still reached.

**Macroexpansion budget is diagnosed, not a hang, and is directly tested.** `macroexpand<MaxNodes,MaxList>(d, arena, table, max_expansions = default_max_expansions)` loops calling `macroexpand_1` until it reports no further expansion or the budget (`default_max_expansions = 64`) is exhausted, at which point it returns a `parse_error{{}, "macroexpand: expansion budget exceeded"}`. None of the six real macros can loop (each rewrites to `if`/`progn`/`let`/`cond`, none of which are themselves macro names), so exercising this path needed a test-only, deliberately non-terminating macro (`expand_loop_forever`, `expander.test.cpp`, registered in a small ad-hoc table passed to `macroexpand`'s explicit-table overload) — `ExpanderTest - MacroexpandBudgetExceededIsDiagnosedError` drives it with a tiny budget (3) and checks the result is an error, not a compiler hang. Both `macroexpand_1` and `macroexpand` have a `std::span<host_macro const>`-taking overload (used by the test) and a convenience overload defaulting to `default_macro_table()` (used everywhere else, including `expand_datum`/`expand_and_elaborate`).

**Merge criteria**, all satisfied: expansion-shape tests for all six macros (via `macroexpand_1`, checking the resulting datum shape — head symbol, arity, nesting); the end-to-end `(cond (nil 1) (t 2))` ⇒ `2` both as a compile-time `static_assert` (mirroring L11's `eval_direct.test.cpp` merge-criteria pattern exactly: read → `expand_and_elaborate` → `eval_direct`, no evaluator or elaborator changes) and as a runtime `TEST_CASE`. Additional end-to-end `TEST_CASE`s cover `when`/`unless`/`and`/`or`/`case` (including `and`'s short-circuit not evaluating an unbound variable, and `case`'s multi-key and default-clause dispatch) through the full read→expand→elaborate→eval pipeline, using L11's existing `nil`/`t` truthiness and `if`/`let`/`progn` semantics with zero changes to `eval_direct.hpp` or `elaborate.hpp`.

`make compile`, `make test` (466/466 passed, including 25 new `ExpanderTest` cases plus the merge-criteria `static_assert`), and `make lint` all passed clean at landing (`pre-commit run -a` reformatted the two new files once during authoring via `clang-format`, as normal for brand-new files; no pre-existing file was touched). `git diff --stat -- src/smd/smdscheme` is empty (frozen tree untouched); `git diff --stat -- src/smd/smdlisp/reader src/smd/smdlisp/elaborator src/smd/smdlisp/closure` is also empty (L12/L14-L16's lanes, if in flight concurrently, are undisturbed) — the only changes are the new `src/smd/smdlisp/macroexpand/` directory and the two-line `add_subdirectory`/link edit to `src/smd/smdlisp/CMakeLists.txt`. No blog deliverable for this step (phase 20 arrives at L19, `defmacro`, per the plan's phase table). One divergence doc filed: `docs/divergences/DIV-0006-macro-temp-name-capture.md` (reserved-name capture risk in `or`/`cond`/`case`, status `open`, revisit at L18/L19). Next free divergence number is **DIV-0007** (a parallel L12 worker may have already claimed DIV-0005 or DIV-0006 by the time this merges — check `docs/divergences/` for the actual latest number before filing).
