...
constexpr auto my_ffi_func(std::span<value<core_type<32, 16>> const> args)
    -> result<value<core_type<32, 16>>> {
    if (args.size() != 1)
        return result<value<core_type<32, 16>>>{
            parse_error{{}, "ffi arity mismatch"}};
    if (!std::holds_alternative<int>(args[0]))
        return result<value<core_type<32, 16>>>{
            parse_error{{}, "ffi type error"}};
...
TEST_CASE("FFITest - Basic") {
    ...
    e.define("add-100", value<core_type<32, 16>>{
                            foreign_function<core_type<32, 16>>{my_ffi_func}});
