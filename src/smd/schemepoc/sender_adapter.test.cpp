...
TEST_CASE("SenderAdapter - then") {
    auto s = smd::schemepoc::sender_v::then(smd::schemepoc::sender_v::just(21),
                                            [](int x) { return x * 2; });
    auto result = smd::schemepoc::sender_v::sync_wait(s);
    REQUIRE(std::get<0>(result.value()) == 42);
}
...
TEST_CASE("SenderAdapter - when_all") {
    auto all_s = smd::schemepoc::sender_v::when_all(
        smd::schemepoc::sender_v::just(21), smd::schemepoc::sender_v::just(2));

    auto s = smd::schemepoc::sender_v::then(all_s,
...
TEST_CASE("SenderAdapter - Maps Dynamically to SchemePoC Values") {
    ...
    // Simulate apply
    auto s =
        smd::schemepoc::sender_v::then(all_s, [](res_t a, res_t b) -> res_t {
            if (!a.has_value() || !b.has_value())
                return smd::schemepoc::parse_error{{}, "error"};
            if (!std::holds_alternative<int>(a.value()) ||
                !std::holds_alternative<int>(b.value()))
                return smd::schemepoc::parse_error{{}, "type error"};
            return res_t{
