// src/smd/kit/parser/cursor.hpp                                     -*-C++-*-
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
// Moved here in step B2 from src/smd/cl/reader/cursor.hpp, which becomes a
// forwarding shim over this file. cursor, parse_state and advance_while were
// already free of anything Lisp-specific -- the reader's own read_context.hpp
// said so before this move -- and B2's context-threaded parser<F> needed to
// compose over the same cursor and parse_state the reader already had, so
// the kit is where both now live.
#ifndef SRC_SMD_KIT_PARSER_CURSOR_HPP
#define SRC_SMD_KIT_PARSER_CURSOR_HPP
...
/// stopping at end of input. This is the parser layer's character-level
/// iteration primitive; higher layers compose it rather than writing their
/// own character loops.
template <class Pred>
[[nodiscard]] constexpr auto advance_while(cursor cur, Pred keep_going)
    ...
    // scanning layers above are built from.
