...
// Shim smoke test (step R8): the substantive result monad-law tests moved
// to src/smd/kit/foundation/result.test.cpp with the definition. This file
// only verifies that the forwarding shim compiles and that the forwarded
// names are usable from smd::cl::foundation exactly as before, including
// the value_type alias and equality that result_instances.hpp depends on.
//
// and_then is no longer among those names: it now sits beside the Monad
// instance it is a spelling of, so its shim case is in
// result_instances.test.cpp.
