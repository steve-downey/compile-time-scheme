...
constexpr auto ffi_print_and_add(std::span<smd::schemepoc::value<Core> const> args) 
    -> smd::schemepoc::result<smd::schemepoc::value<Core>> {
    if (args.size() != 2) return smd::schemepoc::parse_error{{}, "ffi arity mismatch"};
...
int main() {
    ...
    env.define("print-and-add", smd::schemepoc::value<Core>{
