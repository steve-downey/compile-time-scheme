# Step B2 — `smd::kit::parser`, with `bind` and a threaded context
...
Add `kit.parser` to the exported target list in the top-level `CMakeLists.txt`
that A4 rewrote, so the new module is installed like the rest.
...
template <class P, class F> [[nodiscard]] constexpr auto bind(P p, F f);
```

`bind(p, f)` runs `p`, and on success runs `f(value)` — which returns a
parser — at the remaining cursor with the same context. `f` takes the value
alone, not the value and the context: the context arrives when the returned
parser is run, and that is what makes the monad laws statable.
...
**`src/smd/kit/parser/parser.test.cpp`** and `parse_context.test.cpp` and
`cursor.test.cpp` — double-include, bootstrap test, then **law tests first**,
per `docs/CODING_RULES.md`'s "add law-focused tests before performance tests"
and `docs/cpp-rules.md`'s "law tests … come before any other substantive test":
...
CMakeLists.txt                               (export kit.parser)
src/smd/cl/reader/cursor.hpp                 (becomes a forwarding shim)
src/smd/cl/reader/detail/read_context.hpp    (reader_context concept; token_p)
...
`src/smd/cl/reader/cursor.test.cpp` is **not** in scope: the shim keeps the name
alive precisely so it does not have to change.
...
grep -c 'static_assert' src/smd/kit/parser/parser.test.cpp
...
The third must return nothing — `no_context` is never a default. The fourth
still shows `scan_token` called, unmodified. The fifth shows `read_radix_number`
no longer reaching for `and_then` directly; `read_sharpsign`, still unconverted,
may.
...
git add -A
git commit -F - <<'EOF'
kit: a parser layer with bind, and its first Common Lisp client
