...
- [ ] A1 — `tmp/plan/step-A1.md` — a `prin1`-shaped printer for `cl` datums, proved against SBCL in the same step
- [ ] A2 — `tmp/plan/step-A2.md` — the reader-layer cl-vs-SBCL differential replaces the `smdlisp` one
- [ ] A3 — `tmp/plan/step-A3.md` — freeze both iterations as tags; move their architecture prose to a pinned history doc
- [ ] A4 — `tmp/plan/step-A4.md` — retire the dead trees' examples, install-test coverage, and export list
- [ ] A5 — `tmp/plan/step-A5.md` — delete `smdscheme` and `smdlisp` from trunk
...
- [ ] B2 — `tmp/plan/step-B2.md` — `smd::kit::parser`: context-threaded parser and `bind`, with `read_radix_number` as its first client
- [ ] B3 — `tmp/plan/step-B3.md` — intertoken space and comments onto the layer
- [ ] B4 — `tmp/plan/step-B4.md` — choice and bounded repetition; `read_delimited` retires the D15 ladder
