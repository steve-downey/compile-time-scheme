...
// the bare repo) it is the bare repo directory itself.  It also resolves any
...
// If the user fetches from a secondary remote the local head moves forward but
// origin's tracking ref stays behind, causing Antora to read stale content.
// We therefore sync every remote-tracking ref for the current branch to the
// local head before Antora aggregates content.
...
module.exports.register = function () {
  this.once('playbookBuilt', ({ playbook }) => {
    ...
    // rather than refs/heads/<branch>.  Sync the origin tracking ref from the
    // local head so Antora can find the branch even when it has never been pushed.
