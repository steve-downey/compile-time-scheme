...
- **`(block nil ...)` is diagnosed, not accepted.**
  The atom pass turns `NIL` and `T` into constants before the role pass runs, so neither is available as a block name. ANSI uses `(block nil ...)` for `loop`'s implicit block, so this will need answering before iteration macros land. Classify it before pinning it.
