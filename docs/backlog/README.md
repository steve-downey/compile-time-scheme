...
| [BL-0008](BL-0008-lint-autofix-lands-unstaged.md) | `make lint`'s auto-fixing hooks pass on the rerun, so their repairs miss the commit | open — three occurrences in one fan-out; wants a guard, not another warning |
| [BL-0009](BL-0009-alternative-kit-membership.md) | Decide whether `alternative` still belongs in the kit, given nothing uses it | open — filed as BL-0008 and renumbered 2026-09-11; BL-0008 was already taken on `cl-parser-combinators` |
| [BL-0010](BL-0010-derive-monad-fmap-captures-by-reference.md) | `derive_monad`'s derived `fmap` captures by reference, which dangles for a deferred instance | open — no in-tree caller can reach it; `parser<F>` can, and overrides it |
| [BL-0011](BL-0011-a-skipped-oracle-reads-as-a-passing-one.md) | Make a skipped SBCL oracle distinguishable from an agreeing one, and record which build agreed | open — a green matrix currently means either forty-six comparisons or none |
| [BL-0012](BL-0012-plan-spot-checks-ship-unexecuted.md) | Execute every path and spot-check command in a plan before dispatching its first worker | open — seven instances in one fan-out, none found by the plan's author |
| [BL-0014](BL-0014-posts-cite-internal-identifiers.md) | Take step names, decision numbers and divergence numbers out of the posts' running text | open — scheduled after the Phase B merge; scope question to settle first |
| [BL-0015](BL-0015-per-step-number-reservations.md) | Stop reserving divergence and blog-phase numbers per step before the step knows it needs them | open — split out of the step-slug work; four unspent divergence numbers read as four missing records |
