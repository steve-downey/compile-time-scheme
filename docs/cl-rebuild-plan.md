...
Behind that sits the claim the whole rebuild rests on: an arena tree stored children-before-parent is its own base functor's fixed point — the recursive position is an index — and index order is a topological order, so every pass over the tree is a linear fold.
A catamorphism ascends, an inherited-attribute scan descends through the transpose of the child lists, and `traverse` over the result applicative covers the per-element effectful case; error propagation is a scheme's short-circuiting carrier, never an `if (!r.has_value()) return r;` ladder.
...
**R4 — elaborator as schemes.**
Three passes, each a named scheme: `traverse` over the result applicative for the atoms, the inherited-attribute scan for the roles, and a paramorphism for the emission.
`para` rather than `cata` exactly where the algebra reads a child as *syntax* — a name in head position, a binding list — which is DIV-0016's driver with the machinery it actually needs on a columnar tree; no Mendler variant, because over a materialized column the recurse-knob has nothing left to turn.
...
- Whether `smdlisp` is eventually retired or kept permanently as the pivot's artefact.
  ...
  **Resolved 2026-08-23: retired.** Decision D32 (`docs/cl-language-scoping.md`) overrides this question's own premise — R6's conformance corpus alone was not judged sufficient reason to keep `smdlisp` in the worktree; the deciding reason was agent friction with the never-edited rule, not oracle coverage. `smdlisp` was deleted from trunk at step A3 of `tmp/plan/`'s tree-retirement plan and is preserved at the annotated tag `iteration/smdlisp-final`, as a source of source programs only — D16 already required expectations to come from SBCL or the specification, never from `smdlisp` itself.
