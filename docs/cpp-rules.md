...
- **A recursion over a tree is a catamorphism.**
  Consume recursive structure by passing an algebra to the substrate's schemes — `cata`/`para` and their short-circuiting forms for synthesized values, `scan_down` for inherited ones, the Foldable and Traversable instances for element folds — not by hand-written recursive descent.
