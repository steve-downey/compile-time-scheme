...
Measured with `g++-14 -std=c++26`, the derived `fold_left` over `pair_box<int>{1, 2}`:
...
Unspecified evaluation order is unspecified per compiler as well as per optimization level, and the measurements above are `g++-14`, which is not this project's `gcc-16` baseline — it is what a container without access to the toolchain PPAs can install.
