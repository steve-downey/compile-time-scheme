...
<div class="abstract" id="org64a86b5">
<p>
...
But I have to be more careful than that, because this series solved this problem inside a fold once already. Phase 7's interpreter is a Mendler-style fold (Mendler, Nax Paul, 1987), which hands the algebra its own `recurse` and so lets it descend into the arm it chose and into no other. "Evaluation can't be a fold" would have been news to me in June.

What killed it is that the tree changed underneath it. That interpreter needed a pointer tree, and a `tagged_tree` has none to chase and no descent to decide: children's results are computed by position, before the parent is looked at, so the knob has nothing left to turn. `tagged_tree_schemes.hpp` offers `cata` and `para` and deliberately no Mendler variant for that reason. Phase 29 is where I go back over that properly, with what it costs to get the knob back and what reading it all again turned up underneath.
