...
#+TITLE: Building a Compile-Time Scheme in C++26: Phase 41 - Skipping Space, and a Law That Would Not Finish
#+AUTHOR: Steve Downey
#+DATE: 2026-09-16
...
~src/smd/kit/parser/~ gains ~satisfy~, ~char_p~ and ~skip_many~, a repetition that discards every value and treats exhaustion as success.
~skip_intertoken_space~ becomes one ~skip_many~ over a parser that handles whichever of the three skippable syntaxes applies.
...
~skip_many(skip_many(p))~ never comes back, and GCC reports that as an operation count exceeding 33554432, which is what an infinite loop looks like when the loop runs in the constant evaluator.
~skip_many~ is itself a parser that succeeds without consuming once its inner parser is spent, so an outer ~skip_many~ is handed a success at the same cursor forever.
...
The comment skipper did not become a combinator at all. A recursive function with a depth parameter, and it never touches ~skip_many~.
Two new cases pin what an unterminated ~#|~ does, flat and nested. Nothing had checked that before, so the conversion had nothing to be measured against until the step wrote the measure.
Both cases went into a file the step's own spot check says must not change.
And the premise ("the reader's last two raw loops") is short by three. ~forms.hpp~, ~text.hpp~ and ~token.hpp~ each still hold one, each under a comment making the same claim this step set out to stop making.
...
[[file:index.org][↑ Series Index]] | [[file:phase-40-one-bind.org][Phase 40 - One bind, and the Namespace It Could Not Be Declared In ←]]
#+html: </nav>
...
#+transclude: [[orgit-file:~/src/compile-time-scheme/main::blog/phase-41::src/smd/kit/parser/repeat.hpp::013cfe3b-6dfa-4f4b-b715-a59917e7715b]] :lines 2- :src cpp :end "013cfe3b-6dfa-4f4b-b715-a59917e7715b end"
...
#+transclude: [[orgit-file:~/src/compile-time-scheme/main::blog/phase-41::src/smd/cl/reader/detail/skip.hpp::2ce9b1bf-8125-40fa-8496-fb96580aab21]] :lines 2- :src cpp :end "2ce9b1bf-8125-40fa-8496-fb96580aab21 end"
...
- "Substrate generic algorithm: the reader's token accumulator... this is the token layer's one primitive loop, built directly on cursor." (~token.hpp~)
