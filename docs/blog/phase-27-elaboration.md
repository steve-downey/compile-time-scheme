...
<div class="abstract" id="orga295a3f">
<p>
...
Its three passes are three named schemes from phase 26's foundation (<code>traverse</code>, <code>scan_down</code>, <code>para_short</code>), and none of them calls itself, because the tree only <i>looks</i> recursive.
...
[↑ Series Index](index.md) | [Phase 26 - Reading More Than It Can Run ←](phase-26-reader-core-ast.md)
...
Which means ascending index order *is* a bottom-up catamorphism (Meijer, Erik and Fokkinga, Maarten and Paterson, Ross, 1991) (every node visited after all of its children), and descending index order is the mirror, every node visited after its parent. Phase 26 shipped both directions as named schemes in `tagged_tree_schemes.hpp`, with law tests: `cata` and its relatives ascend, `scan_down` descends, and each one is a linear pass over the node array. So this step writes no traversal of its own; every pass names the scheme it is and supplies an algebra, and nesting depth stops being a resource. The only capacity that matters is the node count, which is a number the tree already knows.
...
Each node asks its parent what position it occupies: an expression, quoted data, the head of a form, or unreached. Asks, literally. The pass is one `scan_down` call, the inherited-attribute scheme from phase 26: the scheme hands each step its parent's finished plan through the tree's link column, `role_of` reads that plan, `form_of` classifies the node's own head, and the step writes exactly one entry, its own. So the pass is a fold, and its result is the plan column, one plan per node. The link column is what makes that shape possible. Without a way to find its parent, a node can only be told what it is, by each parent scattering roles into its children's entries; the link column is the transpose of the child lists, and a child that can ask is the reason it exists. And descending order makes the asking safe: a parent's index always exceeds its children's, so its plan is finished before any child reads it.

The pass is total and there's no effect to thread, so the scheme is the plain one; it can't fail. Reachability falls out all the same. `role_of` answers `evaluate` for the root, `unreachable` for a node no branch names, and a child of an unreached parent inherits it. A datum nothing reaches keeps `unreachable`, and the emission pass emits nothing for it. The reachability analysis in this file is `role_of`'s fallback answer.
...
Emission ascends as `para_short` with carrier `int`: the algebra is handed a source layer whose child slots already hold the children's emitted core indices, and returns the node's own. Tree indices in, core indices out. No pass carries a map from source node to core node, because the paramorphism's accumulator is that map (entry *i* is node *i*'s core index, one value per node), and the algebra never sees it; a node's children arrive already translated, without anyone going and looking for them.

Plain `cata` would have been enough if every child were a subexpression, and one is not: a call's head is read as syntax, a name (which is why phase 26 put a `core_call`'s callee in the tag instead of a child). So the algebra also takes the tree and the node's own index, and reaches back to the source child list to pull the callee's symbol out. That reach is everything `para` adds to `cata`, and the whole reason emission uses it.
...
The next step's question is whether a CPS evaluator over the same tree can stay in the same shape &#x2014; named schemes, no recursive descent, because the tree only *looks* recursive. I don't see why it couldn't. That isn't the same as knowing.
