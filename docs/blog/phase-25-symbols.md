...
<div class="abstract" id="org2caed73">
<p>
...
An entry is an offset and a length into a pooled block of characters, a flag saying whether name lookup can reach it, and three slots. The storage is two of R1's `static_vector` containers: the entries, and the character pool the names live in. The pool is a column grown at its end with `append_range`, never written through an index.
...
It carries no capacity, which is what lets a program traffic in ids freely. Recovering a name or a slot requires the owning table: identity gets cheap and portable, and everything else needs a second argument. Which reads like a local trade, and isn't. An id is a position in a column, the table is the column store, and asking for a name is indexing a column. The substrate made the same transposition when it decided data lives in columns grown at the end, and a symbol table is what that move looks like when the data is names.
