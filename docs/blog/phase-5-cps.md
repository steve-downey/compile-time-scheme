<div class="abstract" id="org95f1674">
<p>
By Phase 4, my compile-time compiler could parse and elaborate code, but recursive execution deeply stacked the C++ compiler's internal call limits.
...
Instead of rewriting an entire interpreter loop within C++ structures natively (which often resorts to heavy trampoline mechanics like Reynolds Defunctionalization (Danvy, Olivier, 2006) applied manually), I leverage the C++ compiler's own native optimizer. Over our `core_type` AST, I built `cps_dispatch`.
...
Because Scheme mandates Tail-Call Optimization (meaning infinite recursion without stack overflow is valid loop behavior), CPS architectures like the Rabbit compiler (Steele, Guy L., 1978) historically built mechanical jump tables.
...
By forwarding \`cont\` and \`k\` into the recursive instantiation of \`cps\_dispatch(arena.get(lam.body)&hellip;)\` inside the \`return\` statement, Clang and GCC correctly collapse the compile-time execution frame. A recursive Scheme call executing this branch does not consume extra memory depth, compiling infinite Scheme tail recursion securely in C++.
...
Danvy, Olivier (2006). *Defunctionalization at Work*.
