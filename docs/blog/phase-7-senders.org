...
I achieve this by lowering my CPS Abstract Syntax Tree into deeply nested *Typed Senders*.
#+end_abstract
...
Because my Scheme compiler relies on Continuation-Passing Style—where every operation simply says "do work, then pass the result to this callback"—it matches perfectly with the Sender/Receiver architecture.
...
I am passing deeply nested templates into the C++ compiler.

Where typical dynamic language interpreters rely on heavy switch statements and virtual dispatch (creating cache misses and blocking optimization), my Scheme program compiles into a single, fully-known type topology.
