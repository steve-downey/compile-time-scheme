...
Every phase of this project was a design problem dressed as a language limit. No persistent heap. No virtual dispatch. No allocator in `constexpr`. Each constraint forced a cleaner abstraction, and the accumulation of those abstractions turned out to be a functional compiler running entirely inside the C++ constant evaluator.

This final post reviews each phase, draws out the lessons, and sketches what the next ascent might look like.
...
The parser combinators use immutable cursors: a string view plus an offset. Combining two parsers advances the cursor; failure propagates through `result`. No allocation happens at parse time — the combinator structure is entirely static.
