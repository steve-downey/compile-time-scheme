...
The reader produces a generic tree of lists, atoms, and symbols. It does not know if ~(if #t 1 2)~ is a function call, a variable definition, or a control flow construct—it just sees a list of four items.
...
These branches are handles (~arena_box~) to other nodes allocated in the AST arena, preventing deep recursion overhead.
