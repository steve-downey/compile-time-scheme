...
#+TITLE: Building a Compile-Time Scheme in C++26: Phase 4 - Elaboration and Semantic Analysis
#+AUTHOR: Steve Downey
#+DATE: 2026-06-19

#+begin_abstract
The elaborator transforms raw datum trees into a typed core AST. It recognizes
~if~, ~lambda~, ~quote~, ~define~, ~let~, and ~let*~ — turning unstructured
...
[[file:index.org][↑ Series Index]] | [[file:phase-3-reader.org][Phase 3 - Reader ←]]
#+html: </nav>
...
understands keywords couples syntax to semantics. Every new special form
would mean changing the reader too. Keeping them apart means the reader
never needs to change when the language gains new special forms.
...
[[file:index.org][↑ Series Index]] | [[file:phase-5-fixpoint-trees.org][Next: Phase 5 - Fixpoint Trees →]]
#+html: </nav>
