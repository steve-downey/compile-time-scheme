...
#+TITLE: Building a Compile-Time Scheme in C++26: Phase 40 - One bind, and the Namespace It Could Not Be Declared In
#+AUTHOR: Steve Downey
#+DATE: 2026-09-16
...
#+begin_abstract
This step creates ~smd::kit::parser~: a ~parser<F>~ over a context threaded through every call.
The type is registered as a Monad instance instead of getting a ~bind~ of its own, and ~read_radix_number~ is converted as its first client.
That registration, as the brief illustrated it, does not compile.
~monad_typeclass~ is a variable template in ~smd::kit::foundation~, and a specialization has to be declared in a namespace enclosing the primary template's own.
~smd::kit::parser~ sits beside that namespace instead of inside it.
The step checked that against GCC 16 instead of assuming it, and used the shape ~src/smd/cl/foundation/tagged_tree_instances.hpp~ already carried for the same reason one level down.
~grep 'monad_typeclass<parser'~, the check that was supposed to find the registration, finds nothing, and the step wrote down that no valid spelling could match it.
One valid spelling does. It is not the one that landed, and the one that landed had the precedent.
~read_radix_number~ is now ~bind(token_p, classify_and_add)(cur, ctx)~, with the same two diagnostic strings at the same two positions and ~scan_token~ untouched.
Of the three primitives the step lands, ~bind~ is the only one anything outside the kit's own tests calls.
~monad.hpp~'s doc comment says an instance that defers its continuation has to supply ~join~, ~then~ and ~apply~ itself instead of inheriting them.
Its own example of such an instance is "a parser that stores what to do next".
~parser_monad_map~ inherits all three.
The reader's ~CMakeLists.txt~ gains ~kit.parser~, a real new dependency, recorded as out of declared scope instead of passing unremarked.
...
[[file:index.org][↑ Series Index]] | [[file:phase-39-eight-headers.org][Phase 39 - Eight Headers, and a Comment the Formatter Made False ←]]
#+html: </nav>
...
#+transclude: [[orgit-file:~/src/compile-time-scheme/main::blog/phase-40::src/smd/kit/parser/parser_instances.hpp::606983ed-f5d7-454d-9570-3b510ace3aba]] :lines 2- :src cpp :end "606983ed-f5d7-454d-9570-3b510ace3aba end"
...
#+transclude: [[orgit-file:~/src/compile-time-scheme/main::blog/phase-40::src/smd/cl/reader/detail/read_context.hpp::73be3f9e-5da8-4a69-b19c-b674b55f4e75]] :lines 2- :src cpp :end "73be3f9e-5da8-4a69-b19c-b674b55f4e75 end"
...
The commit message says "pure, map and bind and nothing else, because read_radix_number is what needed them." ~read_radix_number~ needed ~bind~.
...
#+transclude: [[orgit-file:~/src/compile-time-scheme/main::blog/phase-40::src/smd/kit/parser/parser_instances.test.cpp::915487ab-d22a-4335-ada8-fd79b9d159f4]] :lines 2- :src cpp :end "915487ab-d22a-4335-ada8-fd79b9d159f4 end"
...
1474 seconds, one attempt, green on both legs of the build matrix, verification 53 seconds against a 114-kilobyte log. Seventeen files, 1004 insertions, 116 deletions. Twenty-one new ~ctest~ entries across four test files, and twenty-four ~static_assert~s among them, because the project wants the compile-time contracts and the runtime cases both.
