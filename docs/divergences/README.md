...
| DIV-0028 | parser combinator layer has no `cl` client | `process` | **dissolved 2026-09-11 (B8)**, not closed; B1–B7 put `cl`'s reader on `smd::kit::parser`. The two copies the "third client" counted are `smdscheme`'s `parser/`, deleted from trunk at A3, and `compile-time-forth`'s, which was never in this repository — corrected 2026-09-16, the 2026-09-11 note read them as three front ends here. One real client is the point, not a shortfall |
...
| DIV-0034 | `sbcl_differential.test.cpp` validates the corpus, not `cl` | `process` | accepted-permanent; the reader-layer differential the name promised lands separately, in `conformance/reader_differential.test.cpp` (A4, broadened A5) |

There is no DIV-0005; the number was never issued.
There are no DIV-0030 through DIV-0033 either; DIV-0034 is the next number issued after DIV-0029.
