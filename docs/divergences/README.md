...
| DIV-0028 | parser combinator layer has no `cl` client | `process` | **dissolved 2026-09-11 (B8)**, not closed; B1–B7 put `cl`'s reader on `smd::kit::parser`, and Phase A had already deleted the other two front ends, so the "third client" the revisit condition counted to cannot exist. One real client is the point, not a shortfall |
