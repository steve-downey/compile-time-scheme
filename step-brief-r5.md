...
- **The elaborator is three index folds, not a recursive descent, and R5 should stay in that shape.**
  A `tagged_tree` is built children-before-parent, so node indices are a topological order: ascending index order *is* a bottom-up catamorphism, and descending index order *is* a top-down propagation.
  `elaborate.hpp` runs atoms (ascending `traverse`), then roles (descending, total), then emission (ascending `fold_left_short`).
  No pass needs a stack, and none bounds nesting depth.
  A CPS evaluator over the same tree can be written the same way; do not reach for recursive descent because the tree "looks recursive".
- **Three tools, and which job each one actually wants.**
  `traverse` where a structure is threaded through the effect and every element should be visited.
  `fold_left_short` where later work must be *skipped* — in R4 that is emission, because the core tree is a shared arena and continuing past a failure spends its capacity on nodes that are already discarded.
